...
# Module 04 - Stunting Modeling (SIVESNU)
# ==============================================================================
#
# Sequential orchestrator over the eight scripts of module 04. Renders
...
#
# Scope: scripts 04_01 through 04_08.
#   - 04_01  Apply transfer models to SIVESNU; predict nutrient intake
#   - 04_02  Integrate Chispitas supplementation program contribution
#   - 04_03  Calculate bioavailability adjustments (iron, zinc)
#   - 04_04  Calculate FIES food security score (Rasch model)
#   - 04_05  Prepare stunting modeling data (Boruta, PCA, imputation)
#   - 04_06  Fit continuous height-for-age model (survey-weighted LM + GAM)
#   - 04_07  Generate calibrated synthetic population
#   - 04_08  Pre-compute baseline and QPM nutrient and stunting profiles
#
# Outputs per script (under 01_data/02_processed/):
#   - 04_01  transfer/04_01_sivesnu_nutritional_predicted.parquet
#   - 04_02  transfer/04_02_sivesnu_with_chispitas.parquet
#   - 04_03  transfer/04_03_sivesnu_bioavailable.parquet
#   - 04_04  transfer/04_04_sivesnu_fies_rasch.parquet
#   - 04_05  transfer/04_05_sivesnu_stunting_modeling.parquet
...
#   - 04_06  models/04_06_ni_zscore_normalization_params.rds
...
#   - 04_07  scenarios/04_07_synthetic_population_calibrated.parquet
#   - 04_08  scenarios/04_08_baseline_qpm_demographics.parquet
#            scenarios/04_08_baseline_qpm_profiles.parquet
#
# Prerequisites:
#   - Module 03 outputs in 01_data/02_processed/models/
#     (03_0x transfer model RDS files, 03_01 YJ transforms)
...
#     (SIVESNU operational data dictionary, ENSMI stunting prevalence)
#
...
#     (04_01 -> 04_02 -> ... -> 04_08). No parallelization is applied
#     at module level. Intra-script parallelization is limited to
#     multi-threaded ranger engines in 04_05 (impute_rf_parallel,
#     Boruta) and 04_07 (impute_rf_parallel), which use native
#     thread-level parallelism within a single script.
...
#
# Usage:
#   source("02_code/04_nutritional_impact_covariate/_run_all_module04_stunting_sivesnu.R")
#
# ==============================================================================
...
run_pipeline <- function() {
  ...
  qmd_input_dir <- here("02_code", "04_nutritional_impact_covariate")
  ...
  log_path <- file.path(log_dir, "module04_stunting_sivesnu_log.parquet")
  ...
  # transfer/, models/, and scenarios/). Web resource RDS files are not
  # validated by the orchestrator — they are presentation artefacts, not
  # pipeline data.

  script_specs <- list(
    list(
      step     = "04_01",
      filename = "04_01_prepare_sivesnu_nutritional_data.qmd",
      expected_outputs = c(
        file.path(transfer_dir, "04_01_sivesnu_nutritional_predicted.parquet")
      )
    ),
    list(
      step     = "04_02",
      filename = "04_02_integrate_chispitas_supplementation.qmd",
      expected_outputs = c(
        file.path(transfer_dir, "04_02_sivesnu_with_chispitas.parquet")
      )
    ),
    list(
      step     = "04_03",
      filename = "04_03_calculate_bioavailability_adjustments.qmd",
      expected_outputs = c(
        file.path(transfer_dir, "04_03_sivesnu_bioavailable.parquet")
      )
    ),
    list(
      step     = "04_04",
      filename = "04_04_calculate_fies_food_security_score.qmd",
      expected_outputs = c(
        file.path(transfer_dir, "04_04_sivesnu_fies_rasch.parquet")
      )
    ),
    list(
      step     = "04_05",
      filename = "04_05_prepare_stunting_modeling_data.qmd",
      expected_outputs = c(
        file.path(transfer_dir, "04_05_sivesnu_stunting_modeling.parquet"),
        file.path(transfer_dir, "04_05_sivesnu_pre_pca.parquet"),
        file.path(models_dir,   "04_05_pca_transforms.rds"),
        file.path(models_dir,   "04_05_pca_block_definitions.rds"),
        file.path(models_dir,   "04_05_boruta_selected_variables.rds")
      )
    ),
    list(
      step     = "04_06",
      filename = "04_06_model_stunting_continuous_zlen.qmd",
      expected_outputs = c(
        file.path(models_dir, "04_06_ni_zscore_normalization_params.rds"),
        file.path(models_dir, "04_06_ni_bio_nomaiz_normalization_params.rds"),
        file.path(models_dir, "04_06_model_zlen.rds"),
        file.path(models_dir, "04_06_formula_zlen.rds"),
        file.path(models_dir, "04_06_gam_zlen_ni.rds"),
        file.path(models_dir, "04_06_gam_zlen_ni_export.rds")
      )
    ),
    list(
      step     = "04_07",
      filename = "04_07_generate_synthetic_population.qmd",
      expected_outputs = c(
        file.path(scenarios_dir, "04_07_synthetic_population_calibrated.parquet")
      )
    ),
    list(
      step     = "04_08",
      filename = "04_08_precompute_baseline_qpm_profiles.qmd",
      expected_outputs = c(
        file.path(scenarios_dir, "04_08_baseline_qpm_demographics.parquet"),
        file.path(scenarios_dir, "04_08_baseline_qpm_profiles.parquet")
      )
    )
  )
  ...
  notify_telegram(sprintf(
    "*Module 04 \u2014 Stunting Modeling (SIVESNU)* \u2014 started\nRun ID: `%s`\nScripts: %d (sequential)",
    run_id,
    length(script_specs)
  ))
  ...
  message("")
  message(strrep("=", 70))
  message("Module 04 \u2014 Stunting Modeling (SIVESNU) (sequential) \u2014 Execution Summary")
  message(strrep("=", 70))
