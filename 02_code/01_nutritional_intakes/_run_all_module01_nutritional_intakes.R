...
# ==============================================================================
#
# Sequential orchestrator over the four scripts of module 01. Renders
...
#
# Scope: scripts 01_01 through 01_04.
#   - 01_01  Load and prepare base data
#   - 01_02  Calculate household nutrient intake
#   - 01_03  Estimate age in months from INE birth registry
#   - 01_04  Calculate individual nutrient intake
#
# Outputs per script (under 01_data/02_processed/transfer/):
#   - 01_01  incap_integrated, household_socioeconomic,
#            individuals_demographic, encovi_alimentos,
#            food_equivalences (parquets) + reference_lists (rds)
#   - 01_02  household_intake_total, household_intake_maize,
#            household_intake_plantbased, household_intake_animalbased
#   - 01_03  individuals_with_age_months, ine_birth_proportions
#   - 01_04  individual_nutritional_intake_complete
#
# Prerequisites:
#   - Raw parquets from ENCOVI 2023 and INCAP 2014
#     (01_data/01_raw/)
#   - External tables: food equivalences, PDCAAS, INE birth registry
#     (01_data/03_external/)
#
...
#     (01_01 -> 01_02 -> 01_03 -> 01_04). Heavy operations are
#     vectorized tidyverse pipelines or inherently sequential greedy
#     algorithms (quota-based month assignment, sibling separation
#     correction).
...
#
# Usage:
#   source("02_code/01_nutritional_intakes/_run_all_module01_nutritional_intakes.R")
#
# ==============================================================================
...
safe_render_script <- function(script_spec, qmd_input_dir) {
  ...
  # Captured before the tryCatch so failing scripts report real elapsed time
  ...
  tryCatch(
    {
      ...
      #
      # Framework outputs are data files, so the HTML by-product of
      # quarto_render() is removed on exit, whether the render succeeds
      # or fails.
      ...
      #
      # NO_COLOR disables the ANSI escape sequences Quarto emits via
      # stderr, so messages appear in the IDE's default text color.
...
run_pipeline <- function() {
  ...
  qmd_input_dir <- here("02_code", "01_nutritional_intakes")
  ...
  log_path <- file.path(log_dir, "module01_nutritional_intakes_log.parquet")
  ...
  script_specs <- list(
    list(
      step     = "01_01",
      filename = "01_01_load_and_prepare_base_data.qmd",
      expected_outputs = c(
        file.path(transfer_dir, "01_01_incap_integrated.parquet"),
        file.path(transfer_dir, "01_01_household_socioeconomic.parquet"),
        file.path(transfer_dir, "01_01_individuals_demographic.parquet"),
        file.path(transfer_dir, "01_01_encovi_alimentos.parquet"),
        file.path(transfer_dir, "01_01_food_equivalences.parquet"),
        file.path(transfer_dir, "01_01_reference_lists.rds")
      )
    ),
    list(
      step     = "01_02",
      filename = "01_02_calculate_household_intake.qmd",
      expected_outputs = c(
        file.path(transfer_dir, "01_02_household_intake_total.parquet"),
        file.path(transfer_dir, "01_02_household_intake_maize.parquet"),
        file.path(transfer_dir, "01_02_household_intake_plantbased.parquet"),
        file.path(transfer_dir, "01_02_household_intake_animalbased.parquet")
      )
    ),
    list(
      step     = "01_03",
      filename = "01_03_estimate_age_in_months.qmd",
      expected_outputs = c(
        file.path(transfer_dir, "01_03_individuals_with_age_months.parquet"),
        file.path(transfer_dir, "01_03_ine_birth_proportions.parquet")
      )
    ),
    list(
      step     = "01_04",
      filename = "01_04_calculate_individual_intake.qmd",
      expected_outputs = c(
        file.path(transfer_dir, "01_04_individual_nutritional_intake_complete.parquet")
      )
    )
  )
  ...
  notify_telegram(sprintf(
    "*Module 01 \u2014 Nutrient Intakes* \u2014 started\nRun ID: `%s`\nScripts: %d (sequential)",
    run_id,
    length(script_specs)
  ))

  # Fail-fast sequential execution: a failure stops dispatching the
  # remaining scripts and preserves the log rows produced so far.
  ...
  message("")
  message(strrep("=", 70))
  message("Module 01 \u2014 Nutrient Intakes (sequential) \u2014 Execution Summary")
  message(strrep("=", 70))
